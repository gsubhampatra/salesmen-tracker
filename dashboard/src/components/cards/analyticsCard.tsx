        ...
        <div className="relative flex-shrink-0 group">
          ...
          <div className="relative p-3 border rounded-full shadow-md bg-gradient-to-br from-white to-blue-50 border-blue-100/50">
            {React.cloneElement(icon as React.ReactElement<any>, {
              ...
              height: 24,
              strokeWidth: 1.5,
              className: "text-blue-600 transform transition-transform duration-500 group-hover:scale-110"
            })}
          </div>
        </div>
...
export const TotalLocationsManagedCard: React.FC<{ data: { totalLocations?: number } }> = ({ data }) => (
  <StatCard
...
export const TotalVisitsMadeCard: React.FC<{ data: { totalVisits?: number } }> = ({ data }) => (
  <StatCard
    title="Total Visits"
...
export const OverallAccuracyCard: React.FC<{ data: { accuracyPercentage?: string } }> = ({ data }) => (
  <StatCard
...
export const AvgVisitDurationCard: React.FC<{ data: { averageVisitDuration?: string } }> = ({ data }) => (
  <StatCard
...
export const TotalOutletsAssignedCard: React.FC<{ data: { totalOutletsAssigned?: number } }> = ({ data }) => (
  <StatCard
...
export const TotalOutletsVisitedCard: React.FC<{ data: { totalOutletsVisited?: number } }> = ({ data }) => (
  <StatCard
    title="Total Outlets Visited"
    value={data.totalOutletsVisited?.toLocaleString() || "—"}
    icon={<BarChart />}
  />
);

export const PeakVisitingHourCard: React.FC<{ data: { peakHours?: number[] } }> = ({ data }) => (
  <StatCard
...
export const AverageVisitDurationsCard: React.FC<{ data: { averageVisitDuration?: string } }> = ({ data }) => (
  <StatCard
    title="Avg Visit Duration"
    value={data.averageVisitDuration ? `${data.averageVisitDuration} min` : "—"}
    icon={<Clock />}
...
export const MostVisitedLocationCard: React.FC<{ data: MostVisitedLocation }> = ({ data }) => (
  <StatCard
...
const AnalyticsDashboard: React.FC<{ data: AnalyticsData & { mostVisitedLocation?: MostVisitedLocation } }> = ({ data }) => {
  return (
    <div className="grid grid-cols-1 gap-6 p-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      <TotalOutletsVisitedCard data={data} />
      <PeakVisitingHourCard data={data} />
      <AverageVisitDurationsCard data={data} />
      {data.mostVisitedLocation && <MostVisitedLocationCard data={data.mostVisitedLocation} />}
    </div>
  );
};
