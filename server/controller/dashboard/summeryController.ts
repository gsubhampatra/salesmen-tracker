...
export async function getLocationAnalytic(req: Request, res: Response) {
  try {
    ...
    const analytics = locations.map((location) => {
      ...
      // List of assigned salesmen
      const assignedSalesmans = assignments.map(
        (assignment) => assignment.SalesMan
      );

      // List of visited salesmen
      const visitedSalesmans = Array.from(
        new Set(visits.map((visit) => visit.SalesMan))
      );
      ...
      const totalDistance = visits.reduce(
        (sum, visit) => sum + visit.scanDistance,
        0
      );
      const accuracyDistance =
        visits.length > 0 ? (totalDistance / visits.length).toFixed(2) : null; // in meters
    ...
    res.json({
      ...
      debug: {
        ...
        locationsWithVisits: analytics.filter((a) => a.outletsVisited > 0)
          .length,
      },
    });
  } catch (error) {
